...
public class WebMvcConfig implements WebMvcConfigurer
{
    ...
    /**
     * 添加一些虚拟路径的映射 静态资源路径和上传文件的路径 如果配置了七牛云上传，则上传路径无效
     *
     * @param registry
     */
    @Override
    ...
    /**
     * 添加一些全局的拦截器，做一些操作 验证用户，设置user到session中等
     *
     * @param registry
     */
    @Override
    ...
    {
        registry.addInterceptor(new ApplicationInterceptor(blogContext, paramService))
            .addPathPatterns("/**")
        ...
        registry.addInterceptor(new ThemeHandlerInterceptor(paramRepository))
            ...
            .excludePathPatterns("/article/comments", "/article/approve", "/a/comments", "/a/approve");
    ...
    {
        return container -> container.addErrorPages(new ErrorPage(HttpStatus.NOT_FOUND, "/error?errorCode=404"),
