...
/**
 * 虽然{@code @NBAuth}也能够处理权限拦截 此处的拦截器更多的是为了处理Session和Cookie的一些逻辑 created by Wuwenbin on 2018/1/23 at 13:41
 *
 * @author wuwenbin
 */
public class AdminInterceptor extends HandlerInterceptorAdapter
{
        ...
        throws Exception
    {
        ...
        if (cookie != null)
        {
            ...
            else if (sessionUser.getDefaultRoleId() == blogContext
                .getApplicationObj(NoteBlogV4.Session.WEBMASTER_ROLE_ID))
            {
                return true;
            }
            else
    ...
    static void handleAjaxRequest(HttpServletRequest request, HttpServletResponse response)
        throws IOException
    {
        // 用户未登录或登录时效过期，请重新登录！
