...
/**
 * 资源监听器 检测{@code @NBAuth}注解的资源存入数据库（默认给网站管理员全部权限，否则无法访问应用） created by Wuwenbin on 2018/7/19 at 22:03
 *
 * @author wuwenbin
 */
@Slf4j
...
public class ResourceListener implements ApplicationListener<ContextRefreshedEvent>
{
    ...
    @Autowired
    public ResourceListener(NBContext context, ParamRepository paramRepository)
    {
                    ...
                    for (Method method : methods)
                    {
                        ...
                        for (String last : lasts)
                        {
                            ...
                            if (method.isAnnotationPresent(NBAuth.class))
                            {
                                ...
                                log.info("资源：[url = '{}'，请求方式：'{}'，媒体类型：'{}'] 扫描到@NBAuth，准备处理...",
                                    url,
                                    Arrays.toString(requestMethods),
                                    produces);
                                NBAuth nbAuth = method.getAnnotation(NBAuth.class);
                                String permission =
                                    isNotEmpty(nbAuth.permission()) ? nbAuth.permission() : nbAuth.value();
    ...
    /**
     * 在参数表中插入一条记录 记录程序是否被初始化过（有没有在初始化界面设置过东西，设置过改为1） 当然，此处是程序第一次启动，当然插入值是未初始化过的：0
     */
    private void setUpAppInitialState()
    {
        NBParam initStatus =
            NBParam.builder().name(INIT_STATUS).value(INIT_NOT).remark("标记用户是否在「个人博客」App 的初始化设置页面设置过").level(0).build();
    ...
    /**
     * 是否包含Controller或者RestController 此处的ultimateTargetClass方法是用来获取被spring的cglib代理类的原始类，这样才能获取到类上面的
     * 注解（因为cglib代理类的原理是继承原始的类成成一个子类来操作）
     *
     * @param bean
     * @return
     */
    ...
    {
        return AopProxyUtils.ultimateTargetClass(bean).isAnnotationPresent(RestController.class)
            || AopProxyUtils.ultimateTargetClass(bean).isAnnotationPresent(Controller.class);
    }
