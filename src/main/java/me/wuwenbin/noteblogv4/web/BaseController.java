...
public abstract class BaseController
{
    ...
    /**
     * 获取 jpa 的排序对象 sort 目前layui dataTable不支持多排序
     *
     * @param page
     * @param <T>
     * @return
    ...
    protected NBR ajaxDone(Supplier<Boolean> ifCondition, Supplier<String> operationInfo)
    {
        ...
        catch (Exception e)
        {
            ...
            String msg =
                operationInfo.get() == null ? "操作出现异常" : operationInfo.get().concat("异常，异常信息：").concat(e.getMessage());
            return NBR.error(msg);
        }
    }
    
    /**
     * 类似如下的判定方式使用此方法： if(ifc){ ifo }else{ elseMsg }
     *
     * @param ifc
     * @param ifo
     * @param elseMsg
