...
public class AuthorityServiceImpl implements AuthorityService
{
    ...
    @Autowired
    public AuthorityServiceImpl(ResourceRepository resourceRepository, NBContext blogContext,
        UserRepository userRepository, ParamRepository paramRepository, UserRoleRepository userRoleRepository,
    ...
    public List<LayuiXTree> findResourceTreeByRoleId(long roleId)
    {
        ...
        treeList.addAll(transTo(all,
            NBSysResource::getName,
            NBSysResource::getId,
            NBSysResource::getPermission,
            hasResources::contains,
            res -> false));
     ...
     * @return
     */
    private List<LayuiXTree> transTo(List<NBSysResource> data, Function<NBSysResource, String> title,
        Function<NBSysResource, Long> value, Function<NBSysResource, String> group,
        Function<NBSysResource, Boolean> checked, Function<NBSysResource, Boolean> disabled)
    {
        
        List<LayuiXTree> treeList = new ArrayList<>();
        data.forEach(res -> {
            LayuiXTree tree = new LayuiXTree(title.apply(res), value.apply(res).toString(), group.apply(res),
                checked.apply(res), disabled.apply(res));
            treeList.add(tree);
        });
        return treeList;
    }
