...
public class MessageServiceImpl implements MessageService
{
    ...
    public Page<NBMessage> findPageInfo(Pageable pageable, MessageQueryBO messageQueryBO)
    {
        return messageRepository.findAll((Specification<NBMessage>)(root, query, criteriaBuilder) -> {
            ...
            {
                predicates
                    .add(criteriaBuilder.like(root.get("clearComment"), "%" + messageQueryBO.getClearComment() + "%"));
            }
            ...
            Join<NBMessage, NBSysUser> userJoin =
                root.join(root.getModel().getSingularAttribute("user", NBSysUser.class), JoinType.LEFT);
