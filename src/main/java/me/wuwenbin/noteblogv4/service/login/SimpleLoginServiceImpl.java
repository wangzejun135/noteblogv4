...
/**
 * 普通登录方法 created by Wuwenbin on 2018/7/21 at 18:05
 * 
 * @author wuwenbin
 */
@Service("simpleLogin")
...
public class SimpleLoginServiceImpl implements LoginService<SimpleLoginData>
{
    ...
    {
        NBSysUser findUser =
            userRepository.findByUsernameAndPasswordAndEnableTrue(data.getBmyName(), data.getBmyPass());
        if (findUser != null)
        {
            ...
            if (remember != null && remember.equals(Boolean.TRUE))
            {
                String cookieValue = Base64.encode(findUser.getUsername())
                    .concat(NoteBlogV4.Session.COOKIE_SPLIT)
                    .concat(findUser.getPassword());
                CookieUtils.setCookie(data.getResponse(),
                    NoteBlogV4.Session.REMEMBER_COOKIE_NAME,
                    cookieValue,
                    15 * 24 * 60 * 60);
            }
            ...
            String redirectUrl = findUser.getDefaultRoleId() == masterRoleId ? NoteBlogV4.Session.MANAGEMENT_INDEX
